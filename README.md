...
<p align="center">
  <img src="resources/icons/app_icon.png" width="120" alt="EdgeVision Logo">
</p>
...
| **算法单一** | 传统算法或深度学习二选一 | ✅ 传统算法 + YOLO 深度学习 + 条码识别融合 |
...
### 🧩 可配置化 Pipeline 引擎
```
通道转换 → 图像增强 → 颜色过滤 → 形态学处理 → 形状筛选 → 直线检测 → 条码识别
```
...
### Pipeline 配置
<!-- TODO: 添加Pipeline配置截图 -->
<!-- ![Pipeline配置](docs/screenshots/pipeline_config.png) -->

### 深度学习目标检测
<!-- TODO: 添加YOLO检测截图 -->
<!-- ![YOLO检测](docs/screenshots/yolo_detection.png) -->

### 云平台看板
<!-- TODO: 添加云平台截图 -->
<!-- ![云平台](docs/screenshots/cloud_dashboard.png) -->
...
| 传统Pipeline延迟 | ~5ms/帧 | Intel i7-12700H |
| 条码识别成功率 | >99% (QR Code) | 标准测试图集 |
| MQTT 上报延迟 | <50ms | 本地 Broker |
| 批量检测吞吐 | ~200张/分钟 | 1920×1080 分辨率 |

> ⚠️ 以上数据为参考值，请根据实际测试结果更新
...
│
├── include/                     # 头文件
│   ├── algorithm/               # 算法层：图像处理、DNN推理、条码识别
│   ├── config/                  # 配置层：Pipeline、ROI、检测参数
│   ├── controllers/             # 控制器层：批量检测、ROI交互、配置管理
│   ├── core/                    # 核心层：Pipeline引擎、MQTT通信、方案管理
│   ├── data/                    # 数据层：区域特征、条码结果、检测报告
...
Pipeline 是 EdgeVision 的核心引擎，采用**步骤链模式**，每个步骤独立配置、顺序执行：
